#!/bin/bash
# Restore windows from CSV session file based on a whitelist of applications

# Configuration
SESSION_FILE="$HOME/.config/smart-session/window-session.csv"
WHITELIST_FILE="$HOME/.config/smart-session/window-whitelist.txt"

# Default whitelist (will be created if file doesn't exist)
DEFAULT_WHITELIST="nemo
terminator"

# Check if CSV session file exists
if [ ! -f "$SESSION_FILE" ]; then
    echo "Error: Session file not found at $SESSION_FILE"
    echo "Run save-all-windows-csv.sh first to create a session."
    exit 1
fi
...
# Parse CSV and restore windows
restored_count=0
skipped_count=0
line_num=0

# Read CSV file line by line, skipping header
tail -n +2 "$SESSION_FILE" | while IFS=, read -r window_id desktop pid process_name window_class window_title cmdline; do
    line_num=$((line_num + 1))
    
    # Remove quotes from fields
    process_name=$(echo "$process_name" | sed 's/^"//;s/"$//')
    window_title=$(echo "$window_title" | sed 's/^"//;s/"$//')
